	...
	public ResourceGen getRevenue() {
		try {
			...
			if (color1.equalsIgnoreCase(color2))
				...
				boost = 1.0505;
			String[] ncities = cityids.toArray(new String[cityids.size()]);
			if (ncities.length <= 10)
				boost += 0.1;
			...
			for (String ca : ncities) {
				...
				// System.out.println("Money: " + (((725.0 * getInteger("commerce",apiRaw)/50.0)
				// +725.0)*getInteger("population",apiRaw)/1000.0)*boost);
				...
				if (powered) {
					...
					if (Integer.parseInt(c.imp_aluminumrefinery) != 0) {
						rg.alum += formula(Integer.parseInt(c.imp_aluminumrefinery), 5, 9) * (1 + 3F / 50)
								* ((hasBauxworks) ? 1.36 : 1);
						rg.monet -= 2500 * Integer.parseInt(c.imp_aluminumrefinery);
						rg.baux -= formula(Integer.parseInt(c.imp_aluminumrefinery), 5, 3)
								* ((hasBauxworks) ? 1.36 : 1);

					}
					if (Integer.parseInt(c.imp_munitionsfactory) != 0) {
						rg.munit += formula(Integer.parseInt(c.imp_munitionsfactory), 5, 18)
								* ((hasArmsStock) ? 1.34 : 1);
						rg.monet -= 3500 * Integer.parseInt(c.imp_munitionsfactory);
						rg.lead -= formula(Integer.parseInt(c.imp_munitionsfactory), 5, 6)
								* ((hasArmsStock) ? 1.34 : 1);
					}
			...
			if (offensivewars != 0 || defensivewars != 0) {
				rg.food -= (pop / 1000F + Integer.parseInt(soldiers) * 0.002);
				...
				rg.food -= (pop / 1000F + Integer.parseInt(soldiers) * 0.00133333333);
			...
			rg.monet += WebColors.getBonusAsInt(color);
			return rg;
		} catch (Exception e1) {
			e1.printStackTrace();
		}
