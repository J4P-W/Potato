...
public class Cmds2 {
	public static void tier(Message m) {
		...
		for(int x = 0; x < a.member_id_list.size(); x++) {
			Nation n = PaWAPI.getNation(a.member_id_list.get(x)+"");
			citycount.add(n.cities);
			if(n.cities > counter)
				counter = n.cities;
		}
		embed.addField("1 city", Collections.frequency(citycount, 1)+"");
		for(int x = 2; x <= counter; x++)
			embed.addField(x+" cities", Collections.frequency(citycount, x)+"");
		c.sendMessage(embed);
	}
	...
	public static void allianceInfo(Message m) throws InterruptedException, ExecutionException {
		...
		Message m2 = c.sendMessage("React with :boom: to check military",embed).get();
		m2.addReaction("\uD83D\uDCA5");
		...
		m2.addReactionAddListener(reactionator);
	}
	...
	public static void nationInfo(Message m) {
		TextChannel c = m.getChannel();
		String[] args = m.getContent().split(" ");
		Nation n = PaWAPI.getNation(args[1]);
