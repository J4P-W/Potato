			...
			public void execute(ArrayList<LWar> data) {
				...
				try {
					for(int x = 0; x < data.size(); x++)
						if(App.mainapi.getNation(data.get(x).attackerID).allianceid.contentEquals(allianceid) || allianceid.equalsIgnoreCase("all")) {
							...
							App.api.getChannelById(channelid).get().asServerTextChannel().get().sendMessage(embed).join();
						}
